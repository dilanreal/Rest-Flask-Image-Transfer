...
baseUrl = 'https://dilanreal2.pythonanywhere.com/'
...
def image_client_get():
    ...
    return json.dumps({'statusCode':200, 'status':True, 'code':'00', 'message':'Successfully displayed image data', 'data':json_string_encode}), 200, {'Content-Type':'application/json'}
...
def image_upload():
    ...
    if allowed_file(getGambar.filename):
        ...
        return json.dumps({'statusCode':200, 'status':True, 'code':'00', 'message':'Image upload was successful! ' + response_pickled}), 200, {'Content-Type':'application/json'}
    else:
         return json.dumps({'statusCode':200, 'status':False, 'code':'01', 'message':'Image format not supported!'}), 200, {'Content-Type':'application/json'}
